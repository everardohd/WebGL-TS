...
function main() {
  ...
  const gl = <WebGLRenderingContext>get3DContext(canvas);
...
//
function initShaderProgram(gl: WebGLRenderingContext, vsSource: any, fsSource: any) {
...
function get3DContext(canvas: HTMLCanvasElement): WebGLRenderingContext | null {
  ...
  for (let i = 0; i < names.length; i++) {
    try {
      context = <WebGLRenderingContext>canvas.getContext(names[i]);
    } catch (e) { }
    if (context) {
      break;
    }
